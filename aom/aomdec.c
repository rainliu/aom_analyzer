...
static const arg_def_t trace_level =
ARG_DEF(NULL, "trace_level", 1, "Trace Level to dump for AOM analyzer");
...
static int main_loop(int argc, const char **argv_) {
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &trace_level, argi)) {
      set_trace_level(arg_parse_int(&arg));
    }else
