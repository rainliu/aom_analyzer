...
static int decode_unsigned_max(struct aom_read_bit_buffer *rb, int max, int level, const char* desc) {
  const int data = aom_rb_read_literal(rb, get_unsigned_bits(max), level, desc);
...
static TX_MODE read_tx_mode(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  return tx_mode;
#else
  return aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "tx_mode_select") ? TX_MODE_SELECT : aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "tx_mode");
...
static void read_inter_mode_probs(FRAME_CONTEXT *fc, aom_reader *r) {
  ...
  for (i = 0; i < NEWMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->newmv_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "newmv_prob");
  for (i = 0; i < ZEROMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->zeromv_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "zeromv_prob");
  for (i = 0; i < REFMV_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->refmv_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "refmv_prob");
  for (i = 0; i < DRL_MODE_CONTEXTS; ++i)
    av1_diff_update_prob(r, &fc->drl_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "drl_prob");
}
...
#else
    return aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "reference_mode_select")
               ? REFERENCE_MODE_SELECT
               : (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "reference_mode") ? COMPOUND_REFERENCE : SINGLE_REFERENCE);
...
static void read_frame_reference_mode_probs(AV1_COMMON *cm, aom_reader *r) {
  ...
  if (cm->reference_mode == REFERENCE_MODE_SELECT)
    for (i = 0; i < COMP_INTER_CONTEXTS; ++i)
      av1_diff_update_prob(r, &fc->comp_inter_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "comp_inter_prob");

  if (cm->reference_mode != COMPOUND_REFERENCE) {
    for (i = 0; i < REF_CONTEXTS; ++i) {
      for (j = 0; j < (SINGLE_REFS - 1); ++j) {
        av1_diff_update_prob(r, &fc->single_ref_prob[i][j], ACCT_STR, AOM_DPRINTF_INFO_CFH, "single_ref_prob");
      }
    }
  }
  ...
  if (cm->reference_mode != SINGLE_REFERENCE) {
    for (i = 0; i < REF_CONTEXTS; ++i) {
      ...
      for (j = 0; j < (FWD_REFS - 1); ++j)
        av1_diff_update_prob(r, &fc->comp_ref_prob[i][j], ACCT_STR, AOM_DPRINTF_INFO_CFH, "comp_ref_prob");
      for (j = 0; j < (BWD_REFS - 1); ++j)
        av1_diff_update_prob(r, &fc->comp_bwdref_prob[i][j], ACCT_STR, AOM_DPRINTF_INFO_CFH, "comp_bwdref_prob");
#else
...
static void update_mv_probs(aom_prob *p, int n, aom_reader *r, int level, const char* desc) {
  int i;
  for (i = 0; i < n; ++i) av1_diff_update_prob(r, &p[i], ACCT_STR, level, desc);
}
...
static void read_mv_probs(nmv_context *ctx, int allow_hp, aom_reader *r) {
  ...
  if (allow_hp) {
    for (i = 0; i < 2; ++i) {
      nmv_component *const comp_ctx = &ctx->comps[i];
      update_mv_probs(&comp_ctx->class0_hp, 1, r, AOM_DPRINTF_INFO_CFH, "class0_hp_prob");
      update_mv_probs(&comp_ctx->hp, 1, r, AOM_DPRINTF_INFO_CFH, "hp_prob");
    }
  }
}
...
static void predict_and_reconstruct_intra_block(
  ...
  uint8_t *dst =
    &xd->plane[plane].dst.buf[(row * xd->plane[plane].dst.stride + col) << tx_size_wide_log2[0]];

  AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "=====Predi (PLANE: %02d, TX_POS: %03d %03d, TX_SIZE: %02dx%02d, TX_TYPE: %02d)========\n", plane, col<<1, row<<1, (tx_size_wide_unit[tx_size] << 1), (tx_size_high_unit[tx_size] << 1), get_tx_type(plane_type, xd, block_idx, tx_size));
  for (int j = 0; j < (tx_size_high_unit[tx_size] << 1); j++){
    for (int i = 0; i < (tx_size_wide_unit[tx_size] << 1); i++){
      AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "%02x ", dst[j*xd->plane[plane].dst.stride + i] & 0xFF);
    }
    AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "\n");
  }
  ...
  /*if (!mbmi->skip)*/ {
    ...
    int16_t max_scan_line = 0;
    int eob = 0;
    AOM_DPRINTF(AOM_DPRINTF_INFO_TOKEN, "=====Token (PLANE: %02d, TX_POS: %03d %03d, TX_SIZE: %02dx%02d, TX_TYPE: %02d)========\n", plane, col<<1, row<<1, (tx_size_wide_unit[tx_size] << 1), (tx_size_high_unit[tx_size] << 1), get_tx_type(plane_type, xd, block_idx, tx_size));
    if (!mbmi->skip){
      eob = av1_decode_block_tokens(cm, xd, plane, scan_order, col, row, tx_size,
        tx_type, &max_scan_line, r, mbmi->segment_id);
    }
    AOM_DPRINTF(AOM_DPRINTF_INFO_COEF,  "=====Coeff (PLANE: %02d, TX_POS: %03d %03d, TX_SIZE: %02dx%02d, TX_TYPE: %02d)========\n", plane, col<<1, row<<1, (tx_size_wide_unit[tx_size] << 1), (tx_size_high_unit[tx_size] << 1), get_tx_type(plane_type, xd, block_idx, tx_size));
    for (int j = 0; j < (tx_size_high_unit[tx_size] << 1); j++){
      for (int i = 0; i < (tx_size_wide_unit[tx_size] << 1); i++){
#if CONFIG_HIGHBITDEPTH
        AOM_DPRINTF(AOM_DPRINTF_INFO_COEF, "%08x ", pd->dqcoeff[j*(tx_size_wide_unit[tx_size] << 1) + i] & 0xFFFFFFFF);
#else
        AOM_DPRINTF(AOM_DPRINTF_INFO_COEF, "%04x ", pd->dqcoeff[j*(tx_size_wide_unit[tx_size] << 1) + i] & 0xFFFF);
#endif
      }
      AOM_DPRINTF(AOM_DPRINTF_INFO_COEF, "\n");
    }
#endif  // CONFIG_LV_MAP
  ...
  AOM_DPRINTF(AOM_DPRINTF_INFO_RECON, "=====Recon (PLANE: %02d, TX_POS: %03d %03d, TX_SIZE: %02dx%02d, TX_TYPE: %02d)========\n", plane, col<<1, row<<1, (tx_size_wide_unit[tx_size] << 1), (tx_size_high_unit[tx_size] << 1), get_tx_type(plane_type, xd, block_idx, tx_size));
  for (int j = 0; j < (tx_size_high_unit[tx_size] << 1); j++){
    for (int i = 0; i < (tx_size_wide_unit[tx_size] << 1); i++){
      AOM_DPRINTF(AOM_DPRINTF_INFO_RECON, "%02x ", dst[j*xd->plane[plane].dst.stride+i] & 0xFF);
    }
    AOM_DPRINTF(AOM_DPRINTF_INFO_RECON, "\n");
  }
...
static int reconstruct_inter_block(AV1_COMMON *cm, MACROBLOCKD *const xd,
  ...
  AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "=====Predi (PLANE: %02d, TX_POS: %03d %03d, TX_SIZE: %02dx%02d, TX_TYPE: %02d)========\n", plane, col<<1, row<<1, (tx_size_wide_unit[tx_size] << 1), (tx_size_high_unit[tx_size] << 1), tx_type);
  for (int j = 0; j < (tx_size_high_unit[tx_size] << 1); j++){
    for (int i = 0; i < (tx_size_wide_unit[tx_size] << 1); i++){
      AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "%02x ", dst[j*pd->dst.stride + i] & 0xFF);
    }
    AOM_DPRINTF(AOM_DPRINTF_INFO_PRED, "\n");
  }
  ...
  for (int j = 0; j < (tx_size_high_unit[tx_size] << 1); j++){
    for (int i = 0; i < (tx_size_wide_unit[tx_size] << 1); i++){
      AOM_DPRINTF(AOM_DPRINTF_INFO_RECON, "%02x ", dst[j*pd->dst.stride + i] & 0xFF);
    }
    AOM_DPRINTF(AOM_DPRINTF_INFO_RECON, "\n");
  }
#else
...
static void decode_token_and_recon_block(AV1Decoder *const pbi,
  ...
  AOM_DPRINTF(AOM_DPRINTF_INFO_SBD, "=====Super-Block Data (INTER: %d, SKIP: %d)===================================\n", is_inter_block(mbmi), mbmi->skip);
...
static void decode_block(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  AOM_DPRINTF(AOM_DPRINTF_INFO_SBH, "=====Super-Block Head (SBH Size %04d %04d)==================================\n", mi_size_wide[bsize]<<2, mi_size_high[bsize]<<2);
  pbi->common.aom_dprintf_sb_flag = 0;
...
#if CONFIG_EC_MULTISYMBOL
    p = (PARTITION_TYPE)aom_read_symbol(r, partition_cdf, PARTITION_TYPES,
                                        ACCT_STR, AOM_DPRINTF_INFO_SBH, "partition");
#else
    p = (PARTITION_TYPE)aom_read_tree(r, av1_partition_tree, probs, ACCT_STR);
#endif
#endif  // CONFIG_EXT_PARTITION_TYPES
  else if (!has_rows && has_cols)
    p = aom_read(r, probs[1], ACCT_STR, AOM_DPRINTF_INFO_SBH, "partition") ? PARTITION_SPLIT : PARTITION_HORZ;
  else if (has_rows && !has_cols)
    p = aom_read(r, probs[2], ACCT_STR, AOM_DPRINTF_INFO_SBH, "partition") ? PARTITION_SPLIT : PARTITION_VERT;
  else
    p = PARTITION_SPLIT;
...
static void decode_partition(AV1Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  if (!cm->aom_dprintf_sb_flag){
    AOM_DPRINTF(AOM_DPRINTF_INFO_SB, "=====Super-Block (SB Pos. %04d %04d)========================================\n", mi_col<<2, mi_row<<2);
    cm->aom_dprintf_sb_flag = 1;
  }
  ...
  if (bsize == BLOCK_64X64) {
    ...
    if (!sb_all_skip(cm, mi_row, mi_col)) {
      cm->mi_grid_visible[mi_row * cm->mi_stride + mi_col]->mbmi.cdef_strength =
          aom_read_literal(r, cm->cdef_bits, ACCT_STR, AOM_DPRINTF_INFO_CFH, "cdef_strength");
    } else {
      cm->mi_grid_visible[mi_row * cm->mi_stride + mi_col]->mbmi.cdef_strength =
          -1;
    }
...
static void setup_segmentation(AV1_COMMON *const cm,
  ...
  seg->enabled = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_enabled");
  ...
  if (frame_is_intra_only(cm) || cm->error_resilient_mode) {
    ...
    seg->update_map = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_update_map");
  }
  if (seg->update_map) {
    if (frame_is_intra_only(cm) || cm->error_resilient_mode) {
      seg->temporal_update = 0;
    } else {
      seg->temporal_update = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_temporal_update");
    }
  }

  // Segmentation data update
  seg->update_data = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_update_data");
  if (seg->update_data) {
    seg->abs_delta = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_abs_delta");
    ...
    for (i = 0; i < MAX_SEGMENTS; i++) {
      for (j = 0; j < SEG_LVL_MAX; j++) {
        int data = 0;
        const int feature_enabled = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_feature_enabled[i][j]");
        if (feature_enabled) {
          av1_enable_segfeature(seg, i, j);
          data = decode_unsigned_max(rb, av1_seg_feature_data_max(j), AOM_DPRINTF_INFO_UDC, "seg_features[i][j]");
          if (av1_is_segfeature_signed(j))
            data = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "seg_feature_signed") ? -data : data;
        }
...
static void setup_loopfilter(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  ...
  lf->filter_level = aom_rb_read_literal(rb, 6, AOM_DPRINTF_INFO_UDC, "filter_level");
  lf->sharpness_level = aom_rb_read_literal(rb, 3, AOM_DPRINTF_INFO_UDC, "sharpness_level");

  // Read in loop filter deltas applied at the MB level based on mode or ref
  // frame.
  lf->mode_ref_delta_update = 0;

  lf->mode_ref_delta_enabled = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "mode_ref_delta_enabled");
  if (lf->mode_ref_delta_enabled) {
    lf->mode_ref_delta_update = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "mode_ref_delta_update");
    if (lf->mode_ref_delta_update) {
      int i;

      for (i = 0; i < TOTAL_REFS_PER_FRAME; i++)
        if (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "ref_deltas_update[i]"))
          lf->ref_deltas[i] = aom_rb_read_inv_signed_literal(rb, 6, AOM_DPRINTF_INFO_UDC, "ref_deltas[i]");

      for (i = 0; i < MAX_MODE_LF_DELTAS; i++)
        if (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "mode_deltas_update[i]"))
          lf->mode_deltas[i] = aom_rb_read_inv_signed_literal(rb, 6, AOM_DPRINTF_INFO_UDC, "mode_deltas[i]");
    }
  }
}
...
static void setup_cdef(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  int i;
  cm->cdef_dering_damping = aom_rb_read_literal(rb, 1, AOM_DPRINTF_INFO_UDC, "cdef_dering_damping") + 5;
  cm->cdef_clpf_damping = aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "cdef_clpf_damping") + 3;
  cm->cdef_bits = aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "cdef_bits");
  cm->nb_cdef_strengths = 1 << cm->cdef_bits;
  for (i = 0; i < cm->nb_cdef_strengths; i++) {
    cm->cdef_strengths[i] = aom_rb_read_literal(rb, CDEF_STRENGTH_BITS, AOM_DPRINTF_INFO_UDC, "cdef_strengths[i]");
    cm->cdef_uv_strengths[i] = aom_rb_read_literal(rb, CDEF_STRENGTH_BITS, AOM_DPRINTF_INFO_UDC, "cdef_uv_strengths[i]");
  }
}
#endif  // CONFIG_CDEF

static INLINE int read_delta_q(struct aom_read_bit_buffer *rb, const char* desc) {
  char desc1[80], desc2[80];
  sprintf(desc1, "%s_delta_q_update", desc);
  sprintf(desc2, "%s_delta_q", desc);
  return aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, desc1) ? aom_rb_read_inv_signed_literal(rb, 6, AOM_DPRINTF_INFO_UDC, desc2) : 0;
}

static void setup_quantization(AV1_COMMON *const cm,
                               struct aom_read_bit_buffer *rb) {
  cm->base_qindex = aom_rb_read_literal(rb, QINDEX_BITS, AOM_DPRINTF_INFO_UDC, "base_qindex");
  cm->y_dc_delta_q = read_delta_q(rb, "y_dc");
  cm->uv_dc_delta_q = read_delta_q(rb, "uv_dc");
  cm->uv_ac_delta_q = read_delta_q(rb, "uv_ac");
...
static InterpFilter read_frame_interp_filter(struct aom_read_bit_buffer *rb) {
  return aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "switchable_interp_filter") ? SWITCHABLE
                             : aom_rb_read_literal(rb, LOG_SWITCHABLE_FILTERS, AOM_DPRINTF_INFO_UDC, "frame_interp_filter");
}

static void setup_render_size(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  cm->render_width = cm->width;
  cm->render_height = cm->height;
  if (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "render_enabled"))
    av1_read_frame_size(rb, &cm->render_width, &cm->render_height, AOM_DPRINTF_INFO_UDC);
}
...
static void setup_frame_size(AV1_COMMON *cm, struct aom_read_bit_buffer *rb) {
  ...
  av1_read_frame_size(rb, &width, &height, AOM_DPRINTF_INFO_UDC);
...
static void setup_frame_size_with_refs(AV1_COMMON *cm,
  ...
  for (i = 0; i < INTER_REFS_PER_FRAME; ++i) {
    if (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "active_ref_idx_enabled")) {
...
// Read the tile width/height
  ...
  while (max_ones-- && aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "log2_tile_cols_bit")) cm->log2_tile_cols++;
  ...
  // rows
  cm->log2_tile_rows = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "log2_tile_rows");
  if (cm->log2_tile_rows) cm->log2_tile_rows += aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "log2_tile_rows_extra_bit");
#if CONFIG_DEPENDENT_HORZTILES
...
#endif
#if CONFIG_LOOPFILTERING_ACROSS_TILES
  cm->loop_filter_across_tiles_enabled = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "loop_filter_across_tiles_enabled");
#endif  // CONFIG_LOOPFILTERING_ACROSS_TILES
...
// tile size magnitude
...
#endif
    pbi->tile_size_bytes = aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "tiles_size_bytes") + 1;
  ...
  if (cm->log2_tile_rows + cm->log2_tile_cols > 0) {
    pbi->tg_start =
        aom_rb_read_literal(rb, cm->log2_tile_rows + cm->log2_tile_cols, AOM_DPRINTF_INFO_UDC, "tile_group_start");
    pbi->tg_size =
        1 + aom_rb_read_literal(rb, cm->log2_tile_rows + cm->log2_tile_cols, AOM_DPRINTF_INFO_UDC, "tile_group_size");
  }
#endif
}
  ...
  for (r = 0; r < tile_rows; ++r) {
    for (c = 0; c < tile_cols; ++c, ++tc) {
      ...
      if (hdr_offset) {
        ...
        if (num_tiles) {
          pbi->tg_start = aom_rb_read_literal(&rb_tg_hdr, num_bits, AOM_DPRINTF_INFO_UDC, "tile_group_start");
          pbi->tg_size = 1 + aom_rb_read_literal(&rb_tg_hdr, num_bits, AOM_DPRINTF_INFO_UDC, "tile_group_size");
#if CONFIG_DEPENDENT_HORZTILES
    ...
    for (tile_col = tile_cols_start; tile_col < tile_cols_end; ++tile_col) {
...
#endif
      AOM_DPRINTF(AOM_DPRINTF_INFO_TIL, "=====Tile (TIL Info. %04d %04d <-> %04d %04d)===============================\n", tile_info.mi_col_start<<2, tile_info.mi_row_start<<2, tile_info.mi_col_end<<2, tile_info.mi_row_end<<2);
      for (mi_row = tile_info.mi_row_start; mi_row < tile_info.mi_row_end;
        ...
        for (mi_col = tile_info.mi_col_start; mi_col < tile_info.mi_col_end;
          ...
          AOM_DPRINTF(AOM_DPRINTF_INFO_LSB, "=====Largest Super-Block (LSB Info. %04d %04d <-> %04d %04d)================\n", mi_col<<2, mi_row<<2, (mi_col+cm->mib_size)<<2, (mi_row+cm->mib_size)<<2);
          cm->aom_dprintf_sb_flag = 0;
...
static void read_bitdepth_colorspace_sampling(AV1_COMMON *cm,
  ...
  if (cm->profile >= PROFILE_2) {
    cm->bit_depth = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "bit_depth") ? AOM_BITS_12 : AOM_BITS_10;
  } else {
  ...
  cm->color_space = aom_rb_read_literal(rb, 3, AOM_DPRINTF_INFO_UDC, "color_space");
  if (cm->color_space != AOM_CS_SRGB) {
    // [16,235] (including xvycc) vs [0,255] range
    cm->color_range = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "color_range");
    if (cm->profile == PROFILE_1 || cm->profile == PROFILE_3) {
      cm->subsampling_x = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "subsampling_x");
      cm->subsampling_y = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "subsampling_y");
      if (cm->subsampling_x == 1 && cm->subsampling_y == 1)
        aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
                           "4:2:0 color not supported in profile 1 or 3");
      if (aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "reserved_bit_set"))
        aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
                           "Reserved bit set");
    } else {
...
static size_t read_uncompressed_header(AV1Decoder *pbi,
  ...
  if (aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "frame_marker") != AOM_FRAME_MARKER)
    aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
                       "Invalid frame marker");

  cm->profile = av1_read_profile(rb, AOM_DPRINTF_INFO_UDC);
  ...
  cm->show_existing_frame = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "show_existing_frame");

  if (cm->show_existing_frame) {
    // Show an existing frame directly.
    const int existing_frame_idx = aom_rb_read_literal(rb, 3, AOM_DPRINTF_INFO_UDC, "existing_frame_idx");
    ...
    if (pbi->seq_params.frame_id_numbers_present_flag) {
      ...
      int display_frame_id = aom_rb_read_literal(rb, frame_id_length, AOM_DPRINTF_INFO_UDC, "display_frame_id");
  ...
  cm->frame_type = (FRAME_TYPE)aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "frame_type");
  cm->show_frame = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "show_frame");
  cm->error_resilient_mode = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "error_resilient_mode");
#if CONFIG_REFERENCE_BUFFER
  if (pbi->seq_params.frame_id_numbers_present_flag) {
    int frame_id_length = pbi->seq_params.frame_id_length_minus7 + 7;
    ...
    cm->current_frame_id = aom_rb_read_literal(rb, frame_id_length, AOM_DPRINTF_INFO_UDC, "current_frame_id");
...
#endif
  if (cm->frame_type == KEY_FRAME) {
    if (!av1_read_sync_code(rb, AOM_DPRINTF_INFO_UDC))
      aom_internal_error(&cm->error, AOM_CODEC_UNSUP_BITSTREAM,
...
#endif  // CONFIG_ANS && ANS_MAX_SYMBOLS
#if CONFIG_PALETTE
    cm->allow_screen_content_tools = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "allow_screen_content_tools");
#endif  // CONFIG_PALETTE
  } else {
    cm->intra_only = cm->show_frame ? 0 : aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "intra_only");
#if CONFIG_PALETTE
    if (cm->intra_only) cm->allow_screen_content_tools = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "allow_screen_content_tools");
#endif  // CONFIG_PALETTE
    if (cm->error_resilient_mode) {
      cm->reset_frame_context = RESET_FRAME_CONTEXT_ALL;
    } else {
      if (cm->intra_only) {
        cm->reset_frame_context = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "reset_frame_context")
        ...
        if (cm->reset_frame_context == RESET_FRAME_CONTEXT_CURRENT)
          cm->reset_frame_context = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "reset_frame_context_extra_bit")
                                        ? RESET_FRAME_CONTEXT_ALL
                                        : RESET_FRAME_CONTEXT_CURRENT;
      }
    }
    ...
    if (cm->intra_only) {
      ...
      pbi->refresh_frame_flags = aom_rb_read_literal(rb, REF_FRAMES, AOM_DPRINTF_INFO_UDC, "refresh_frame_flags");
      setup_frame_size(cm, rb);
      if (pbi->need_resync) {
        memset(&cm->ref_frame_map, -1, sizeof(cm->ref_frame_map));
        pbi->need_resync = 0;
      ...
      for (i = 0; i < INTER_REFS_PER_FRAME; ++i) {
        const int ref = aom_rb_read_literal(rb, REF_FRAMES_LOG2, AOM_DPRINTF_INFO_UDC, "active_ref_idx");
        const int idx = cm->ref_frame_map[ref];
        RefBuffer *const ref_frame = &cm->frame_refs[i];
        ref_frame->idx = idx;
        ref_frame->buf = &frame_bufs[idx].buf;
        cm->ref_frame_sign_bias[LAST_FRAME + i] = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "ref_frame_sign_bias[i]");
#if CONFIG_REFERENCE_BUFFER
        if (pbi->seq_params.frame_id_numbers_present_flag) {
          int frame_id_length = pbi->seq_params.frame_id_length_minus7 + 7;
          int diff_len = pbi->seq_params.delta_frame_id_length_minus2 + 2;
          int delta_frame_id_minus1 = aom_rb_read_literal(rb, diff_len, AOM_DPRINTF_INFO_UDC, "delta_frame_id_minus1");
          int ref_frame_id =
      ...
      cm->allow_high_precision_mv = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "allow_high_precision_mv");
  ...
  if (!cm->error_resilient_mode) {
    cm->refresh_frame_context = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "refresh_frame_context")
  ...
  cm->frame_context_idx = aom_rb_read_literal(rb, FRAME_CONTEXTS_LOG2, AOM_DPRINTF_INFO_UDC, "frame_context_idx");
...
#if CONFIG_DELTA_Q
  {
    ...
    if (segment_quantizer_active == 0 && cm->base_qindex > 0) {
      cm->delta_q_present_flag = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "delta_q_present_flag");
    } else {
      cm->delta_q_present_flag = 0;
    }
    if (cm->delta_q_present_flag) {
      xd->prev_qindex = cm->base_qindex;
      cm->delta_q_res = 1 << aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "delta_q_res");
#if CONFIG_EXT_DELTA_Q
      if (segment_quantizer_active) {
        assert(seg->abs_delta == SEGMENT_DELTADATA);
      }
      cm->delta_lf_present_flag = aom_rb_read_bit(rb, AOM_DPRINTF_INFO_UDC, "delta_lf_present_flag");
      if (cm->delta_lf_present_flag) {
        xd->prev_delta_lf_from_base = 0;
        cm->delta_lf_res = 1 << aom_rb_read_literal(rb, 2, AOM_DPRINTF_INFO_UDC, "delta_lf_res");
      } else {
  ...
  sz = aom_rb_read_literal(rb, 16, AOM_DPRINTF_INFO_UDC, "first_partition_size");
...
static void read_global_motion_params(WarpedMotionParams *params,
  ...
  TransformationType type =
      aom_read_tree(r, av1_global_motion_types_tree, probs, ACCT_STR, AOM_DPRINTF_INFO_CFH, "global_motion_type");
  int trans_bits;
      ...
      if (type != HORTRAPEZOID)
        params->wmmat[6] =
            aom_read_signed_primitive_refsubexpfin(
                ...
                (ref_params->wmmat[6] >> GM_ROW3HOMO_PREC_DIFF), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[6]") *
            GM_ROW3HOMO_DECODE_FACTOR;
      if (type != VERTRAPEZOID)
        params->wmmat[7] =
            aom_read_signed_primitive_refsubexpfin(
                r, GM_ROW3HOMO_MAX + 1, SUBEXPFIN_K,
                (ref_params->wmmat[7] >> GM_ROW3HOMO_PREC_DIFF), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[7]") *
            GM_ROW3HOMO_DECODE_FACTOR;
    case AFFINE:
    case ROTZOOM:
      params->wmmat[2] = aom_read_signed_primitive_refsubexpfin(
                             ...
                             ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[2]") *
      ...
      if (type != VERTRAPEZOID)
        params->wmmat[3] =
            aom_read_signed_primitive_refsubexpfin(
                r, GM_ALPHA_MAX + 1, SUBEXPFIN_K,
                (ref_params->wmmat[3] >> GM_ALPHA_PREC_DIFF), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[3]") *
            GM_ALPHA_DECODE_FACTOR;
      if (type >= AFFINE) {
        if (type != HORTRAPEZOID)
          params->wmmat[4] =
              aom_read_signed_primitive_refsubexpfin(
                  r, GM_ALPHA_MAX + 1, SUBEXPFIN_K,
                  (ref_params->wmmat[4] >> GM_ALPHA_PREC_DIFF), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[4]") *
              GM_ALPHA_DECODE_FACTOR;
        params->wmmat[5] = aom_read_signed_primitive_refsubexpfin(
                               r, GM_ALPHA_MAX + 1, SUBEXPFIN_K,
                               (ref_params->wmmat[5] >> GM_ALPHA_PREC_DIFF) -
                                   (1 << GM_ALPHA_PREC_BITS),
                               ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[5]") *
                               GM_ALPHA_DECODE_FACTOR +
                           (1 << WARPEDMODEL_PREC_BITS);
      } else {
    ...
    case TRANSLATION:
      ...
      params->wmmat[0] =
          aom_read_signed_primitive_refsubexpfin(
              ...
              (ref_params->wmmat[0] >> trans_prec_diff), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[0]") *
          trans_dec_factor;
      params->wmmat[1] =
          aom_read_signed_primitive_refsubexpfin(
              r, (1 << trans_bits) + 1, SUBEXPFIN_K,
              (ref_params->wmmat[1] >> trans_prec_diff), ACCT_STR, AOM_DPRINTF_INFO_CFH, "warped_motion_mat[1]") *
          trans_dec_factor;
    case IDENTITY: break;
    default: assert(0);
  }
...
static int read_compressed_header(AV1Decoder *pbi, const uint8_t *data,
  ...
  for (k = 0; k < SKIP_CONTEXTS; ++k)
    av1_diff_update_prob(&r, &fc->skip_probs[k], ACCT_STR, AOM_DPRINTF_INFO_CFH, "skip_probs");
  ...
  if (frame_is_intra_only(cm)) {
    ...
    for (i = BLOCK_8X8; i < BLOCK_SIZES; ++i) {
      for (j = 0; j < MOTION_MODES - 1; ++j)
        av1_diff_update_prob(&r, &fc->motion_mode_prob[i][j], ACCT_STR, AOM_DPRINTF_INFO_CFH, "motion_mode_prob");
    }
    ...
    for (i = 0; i < INTRA_INTER_CONTEXTS; i++)
      av1_diff_update_prob(&r, &fc->intra_inter_prob[i], ACCT_STR, AOM_DPRINTF_INFO_CFH, "intra_inter_prob");
...
int av1_read_sync_code(struct aom_read_bit_buffer *const rb, int level) {
  return aom_rb_read_literal(rb, 8, level, "av1_sync_code_0") == AV1_SYNC_CODE_0 &&
         aom_rb_read_literal(rb, 8, level, "av1_sync_code_1") == AV1_SYNC_CODE_1 &&
         aom_rb_read_literal(rb, 8, level, "av1_sync_code_2") == AV1_SYNC_CODE_2;
}

void av1_read_frame_size(struct aom_read_bit_buffer *rb, int *width,
                         int *height, int level) {
  *width = aom_rb_read_literal(rb, 16, level, "width_minus1") + 1;
  *height = aom_rb_read_literal(rb, 16, level, "height_minus1") + 1;
}

BITSTREAM_PROFILE av1_read_profile(struct aom_read_bit_buffer *rb, int level) {
  int profile = aom_rb_read_bit(rb, level, "version");
  profile |= aom_rb_read_bit(rb, level, "profile") << 1;
  if (profile > 2) profile += aom_rb_read_bit(rb, level, "profile_extra_bit");
...
void av1_decode_frame(AV1Decoder *pbi, const uint8_t *data,
  ...
  static int frame_no = -1; frame_no++;
  ...
  AOM_DPRINTF(AOM_DPRINTF_INFO_UDC, "=====Uncompressed Data Chunk (Frame No. %04d)===============================\n", frame_no);
  ...
  *cm->fc = cm->frame_contexts[cm->frame_context_idx];
  ...
  xd->corrupted = 0;
  AOM_DPRINTF(AOM_DPRINTF_INFO_CFH, "=====Compressed Frame Header (Frame Size %04d %04d)=========================\n", cm->width, cm->height);
  ...
  if (get_trace_level() & AOM_DPRINTF_INFO_LOOP) {
    AOM_DPRINTF(AOM_DPRINTF_INFO_LOOP, "=====Loop YUV  (Frame Size %04d %04d)=======================================\n", pbi->cur_buf->buf.y_width, pbi->cur_buf->buf.y_height);
    dprintf_yuv(AOM_DPRINTF_INFO_LOOP, pbi->cur_buf->buf.y_buffer, pbi->cur_buf->buf.y_width, pbi->cur_buf->buf.y_height, pbi->cur_buf->buf.y_stride);
    dprintf_yuv(AOM_DPRINTF_INFO_LOOP, pbi->cur_buf->buf.u_buffer, pbi->cur_buf->buf.uv_width, pbi->cur_buf->buf.uv_height, pbi->cur_buf->buf.uv_stride);
    dprintf_yuv(AOM_DPRINTF_INFO_LOOP, pbi->cur_buf->buf.v_buffer, pbi->cur_buf->buf.uv_width, pbi->cur_buf->buf.uv_height, pbi->cur_buf->buf.uv_stride);
  }
  ...
  if (get_trace_level() & AOM_DPRINTF_INFO_FINAL) {
    AOM_DPRINTF(AOM_DPRINTF_INFO_FINAL, "=====Final YUV (Frame Size %04d %04d)=======================================\n", pbi->cur_buf->buf.y_width, pbi->cur_buf->buf.y_height);
    dprintf_yuv(AOM_DPRINTF_INFO_FINAL, pbi->cur_buf->buf.y_buffer, pbi->cur_buf->buf.y_width, pbi->cur_buf->buf.y_height, pbi->cur_buf->buf.y_stride);
    dprintf_yuv(AOM_DPRINTF_INFO_FINAL, pbi->cur_buf->buf.u_buffer, pbi->cur_buf->buf.uv_width, pbi->cur_buf->buf.uv_height, pbi->cur_buf->buf.uv_stride);
    dprintf_yuv(AOM_DPRINTF_INFO_FINAL, pbi->cur_buf->buf.v_buffer, pbi->cur_buf->buf.uv_width, pbi->cur_buf->buf.uv_height, pbi->cur_buf->buf.uv_stride);
  }

  if (get_trace_level() & (AOM_DPRINTF_INFO_LOOP|AOM_DPRINTF_INFO_FINAL)) {
    AOM_DPRINTF(AOM_DPRINTF_INFO_LOOP|AOM_DPRINTF_INFO_FINAL, "=====Frame Done (Frame No %04d)=============================================\n", frame_no);
  }
