...
#else
#define READ_COEFF(prob_name, cdf_name, num, r, level, desc) read_coeff(prob_name, num, r, level, desc);
static INLINE int read_coeff(const aom_prob *probs, int n, aom_reader *r, int level, const char* desc) {
  int i, val = 0;
  for (i = 0; i < n; ++i) val = (val << 1) | aom_read(r, probs[i], ACCT_STR, AOM_DPRINTF_INFO_NULL, NULL);
...
static int token_to_value(aom_reader *const r, int token, TX_SIZE tx_size,
  ...
  switch (token) {
    ...
    case CATEGORY1_TOKEN:
      return CAT1_MIN_VAL + READ_COEFF(av1_cat1_prob, av1_cat1_cdf, 1, r, AOM_DPRINTF_INFO_TOKEN, "token_cat1");
    case CATEGORY2_TOKEN:
      return CAT2_MIN_VAL + READ_COEFF(av1_cat2_prob, av1_cat2_cdf, 2, r, AOM_DPRINTF_INFO_TOKEN, "token_cat2");
    case CATEGORY3_TOKEN:
      return CAT3_MIN_VAL + READ_COEFF(av1_cat3_prob, av1_cat3_cdf, 3, r, AOM_DPRINTF_INFO_TOKEN, "token_cat3");
    case CATEGORY4_TOKEN:
      return CAT4_MIN_VAL + READ_COEFF(av1_cat4_prob, av1_cat4_cdf, 4, r, AOM_DPRINTF_INFO_TOKEN, "token_cat4");
    case CATEGORY5_TOKEN:
      return CAT5_MIN_VAL + READ_COEFF(av1_cat5_prob, av1_cat5_cdf, 5, r, AOM_DPRINTF_INFO_TOKEN, "token_cat5");
    case CATEGORY6_TOKEN: {
      ...
      return CAT6_MIN_VAL + READ_COEFF(av1_cat6_prob + skip_bits, av1_cat6_cdf,
                                       18 - skip_bits, r, AOM_DPRINTF_INFO_TOKEN, "token_cat6");
    }
    default:
...
static int decode_coefs(MACROBLOCKD *xd, PLANE_TYPE type, tran_low_t *dqcoeff,
  ...
  while (more_data) {
    ...
    comb_token = last_pos ? 2 * aom_read_bit(r, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "comb_token_last_pos") + 2
                          : aom_read_symbol(r, coef_head_cdfs[band][ctx],
                                            HEAD_TOKENS + first_pos, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "comb_token_coef_head") +
                                !first_pos;
    if (first_pos) {
#if !CONFIG_EC_ADAPT
    ...
    while (!token) {
      *max_scan_line = AOMMAX(*max_scan_line, scan[c]);
      ...
      comb_token = last_pos ? 2 * aom_read_bit(r, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "comb_token_last_pos") + 2
                            : aom_read_symbol(r, coef_head_cdfs[band][ctx],
                                              HEAD_TOKENS, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "comb_token_coef_head") +
                                  1;
      token = comb_token >> 1;
    }
    ...
    if (token > ONE_TOKEN)
      token +=
          aom_read_symbol(r, coef_tail_cdfs[band][ctx], TAIL_TOKENS, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "token_coef_tail");
#if CONFIG_NEW_QUANT
    dqv_val = &dq_val[band][0];
#endif  // CONFIG_NEW_QUANT
    ...
    *max_scan_line = AOMMAX(*max_scan_line, scan[c]);
    ...
    v = aom_read_bit(r, ACCT_STR, AOM_DPRINTF_INFO_TOKEN, "token_sign") ? -v : v;
    ...
    for (j = AOMMIN(i, cols - 1); j >= AOMMAX(0, i - rows + 1); --j) {
      ...
      const int color_idx =
          aom_read_tree(r, av1_palette_color_index_tree[n - 2],
                        prob[n - 2][color_ctx], ACCT_STR, AOM_DPRINTF_INFO_SBD, "palette_color_index");
