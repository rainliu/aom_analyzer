...
static INLINE int read_uniform(aom_reader *r, int n, int level, const char* desc) {
  const int l = get_unsigned_bits(n);
  const int m = (1 << l) - n;
  const int v = aom_read_literal(r, l - 1, ACCT_STR, level, desc);
  assert(l != 0);
  if (v < m)
    return v;
  else
    return (v << 1) - m + aom_read_literal(r, 1, ACCT_STR, level, desc);
}
#endif  // CONFIG_EXT_INTRA || CONFIG_FILTER_INTRA || CONFIG_PALETTE

#if CONFIG_EC_MULTISYMBOL
static PREDICTION_MODE read_intra_mode(aom_reader *r, aom_cdf_prob *cdf, int level, const char* desc) {
  return (PREDICTION_MODE)
      av1_intra_mode_inv[aom_read_symbol(r, cdf, INTRA_MODES, ACCT_STR, level, desc)];
}
#else
...
static int read_delta_qindex(AV1_COMMON *cm, MACROBLOCKD *xd, aom_reader *r,
  ...
  if ((bsize != BLOCK_LARGEST || mbmi->skip == 0) && read_delta_q_flag) {
...
#else
    abs = aom_read_symbol(r, ec_ctx->delta_q_cdf, DELTA_Q_PROBS + 1, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_q_abs");
#endif
    ...
    if (!smallval) {
      rem_bits = aom_read_literal(r, 3, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_q_rem_bits");
      thr = (1 << rem_bits) + 1;
      abs = aom_read_literal(r, rem_bits, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_q_abs") + thr;
    }

    if (abs) {
      sign = aom_read_bit(r, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_q_sign");
    } else {
      sign = 1;
    }
...
static int read_delta_lflevel(AV1_COMMON *cm, MACROBLOCKD *xd, aom_reader *r,
  ...
  if ((bsize != BLOCK_64X64 || mbmi->skip == 0) && read_delta_lf_flag) {
...
#else
    abs =
        aom_read_symbol(r, ec_ctx->delta_lf_cdf, DELTA_LF_PROBS + 1, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_lf_abs");
#endif
    ...
    if (!smallval) {
      rem_bits = aom_read_literal(r, 3, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_lf_rem_bits");
      thr = (1 << rem_bits) + 1;
      abs = aom_read_literal(r, rem_bits, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_lf_abs") + thr;
    }

    if (abs) {
      sign = aom_read_bit(r, ACCT_STR, AOM_DPRINTF_INFO_SBH, "delta_lf_sign");
    } else {
      sign = 1;
    }
...
static PREDICTION_MODE read_intra_mode_y(AV1_COMMON *cm, MACROBLOCKD *xd,
                                         aom_reader *r, int size_group, int level, const char* desc) {
#if CONFIG_EC_ADAPT
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
...
#if CONFIG_EC_MULTISYMBOL
      read_intra_mode(r, ec_ctx->y_mode_cdf[size_group], level, desc);
#else
...
static PREDICTION_MODE read_intra_mode_uv(AV1_COMMON *cm, MACROBLOCKD *xd,
                                          aom_reader *r,
                                          PREDICTION_MODE y_mode, int level, const char* desc) {
#if CONFIG_EC_ADAPT
  FRAME_CONTEXT *ec_ctx = xd->tile_ctx;
#elif CONFIG_EC_MULTISYMBOL
  FRAME_CONTEXT *ec_ctx = cm->fc;
...
#if CONFIG_EC_MULTISYMBOL
      read_intra_mode(r, ec_ctx->uv_mode_cdf[y_mode], level, desc);
#else
...
static PREDICTION_MODE read_inter_mode(FRAME_CONTEXT *ec_ctx, MACROBLOCKD *xd,
  ...
  if (aom_read(r, mode_prob, ACCT_STR, AOM_DPRINTF_INFO_SBH, "newmv_mode") == 0) {
  ...
  if (aom_read(r, mode_prob, ACCT_STR, AOM_DPRINTF_INFO_SBH, "zeromv_mode") == 0) {
  ...
  if (aom_read(r, mode_prob, ACCT_STR, AOM_DPRINTF_INFO_SBH, "refmv_mode") == 0) {
  ...
  if (mbmi->mode == NEWMV) {
    ...
    for (idx = 0; idx < 2; ++idx) {
      if (xd->ref_mv_count[ref_frame_type] > idx + 1) {
        ...
        if (!aom_read(r, drl_prob, ACCT_STR, AOM_DPRINTF_INFO_SBH, "active_drl_idx_enabled")) {
          mbmi->ref_mv_idx = idx;
...
static MOTION_MODE read_motion_mode(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (last_motion_mode_allowed == OBMC_CAUSAL) {
    motion_mode = aom_read(r, cm->fc->obmc_prob[mbmi->sb_type], ACCT_STR, AOM_DPRINTF_INFO_SBH, "motion_mode");
    ...
    motion_mode =
        aom_read_tree(r, av1_motion_mode_tree,
                      cm->fc->motion_mode_prob[mbmi->sb_type], ACCT_STR, AOM_DPRINTF_INFO_SBH, "motion_mode");
...
#if CONFIG_EC_MULTISYMBOL
  return aom_read_symbol(r, segp->tree_cdf, MAX_SEGMENTS, ACCT_STR, AOM_DPRINTF_INFO_SBH, "segment_id");
#else
...
static TX_SIZE read_selected_tx_size(AV1_COMMON *cm, MACROBLOCKD *xd,
  ...
  const int depth =
      ...
      aom_read_symbol(r, ec_ctx->tx_size_cdf[tx_size_cat][ctx], tx_size_cat + 2,
                      ACCT_STR, AOM_DPRINTF_INFO_SBH, "tx_size_depth");
#else
...
static int read_inter_segment_id(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (seg->temporal_update) {
    ...
    mbmi->seg_id_predicted = aom_read(r, pred_prob, ACCT_STR, AOM_DPRINTF_INFO_SBH, "seg_id_predicted");
...
static int read_skip(AV1_COMMON *cm, const MACROBLOCKD *xd, int segment_id,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    ...
    const int skip = aom_read(r, cm->fc->skip_probs[ctx], ACCT_STR, AOM_DPRINTF_INFO_SBH, "skip");
    FRAME_COUNTS *counts = xd->counts;
...
static void read_palette_mode_info(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (mbmi->mode == DC_PRED) {
    ...
    if (aom_read(r, av1_default_palette_y_mode_prob[bsize - BLOCK_8X8]
                                                   [palette_y_mode_ctx],
                 ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_mode_y")) {
      pmi->palette_size[0] =
          aom_read_tree(r, av1_palette_size_tree,
                        av1_default_palette_y_size_prob[bsize - BLOCK_8X8],
                        ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_size_y") +
          2;
...
#else
      for (i = 0; i < n; ++i)
        pmi->palette_colors[i] = aom_read_literal(r, cm->bit_depth, ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_colors_y[i]");
#endif  // CONFIG_PALETTE_DELTA_ENCODING
      xd->plane[0].color_index_map[0] = read_uniform(r, n, AOM_DPRINTF_INFO_SBH, "color_index_map_y");
  ...
  if (mbmi->uv_mode == DC_PRED) {
    ...
    if (aom_read(r, av1_default_palette_uv_mode_prob[palette_uv_mode_ctx],
                 ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_mode_uv")) {
      pmi->palette_size[1] =
          aom_read_tree(r, av1_palette_size_tree,
                        av1_default_palette_uv_size_prob[bsize - BLOCK_8X8],
                        ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_size_uv") +
          2;
      ...
      for (i = 0; i < n; ++i) {
        pmi->palette_colors[PALETTE_MAX_SIZE + i] =
            aom_read_literal(r, cm->bit_depth, ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_colors_u[i]");
        pmi->palette_colors[2 * PALETTE_MAX_SIZE + i] =
            aom_read_literal(r, cm->bit_depth, ACCT_STR, AOM_DPRINTF_INFO_SBH, "palette_colors_v[i]");
      }
#endif  // CONFIG_PALETTE_DELTA_ENCODING
      xd->plane[1].color_index_map[0] = read_uniform(r, n, AOM_DPRINTF_INFO_SBH, "color_index_map_uv");
...
static void read_intra_angle_info(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (av1_is_directional_mode(mbmi->mode, bsize)) {
    mbmi->angle_delta[0] =
        read_uniform(r, 2 * MAX_ANGLE_DELTA + 1, AOM_DPRINTF_INFO_SBH, "angle_delta_y") - MAX_ANGLE_DELTA;
#if CONFIG_INTRA_INTERP
  ...
  if (av1_is_directional_mode(mbmi->uv_mode, bsize)) {
    mbmi->angle_delta[1] =
        read_uniform(r, 2 * MAX_ANGLE_DELTA + 1, AOM_DPRINTF_INFO_SBH, "angle_delta_uv") - MAX_ANGLE_DELTA;
  }
}
#endif  // CONFIG_EXT_INTRA
    ...
    if (tx_size < TX_32X32 &&
      ...
      if (inter_block) {
        ...
        *tx_type = av1_ext_tx_inv[aom_read_symbol(
            r, ec_ctx->inter_ext_tx_cdf[tx_size], TX_TYPES, ACCT_STR, AOM_DPRINTF_INFO_SBH, "tx_type")];
#else
        ...
        *tx_type = av1_ext_tx_inv[aom_read_symbol(
            ...
            ACCT_STR, AOM_DPRINTF_INFO_SBH, "tx_type")];
#else
        *tx_type = aom_read_tree(
...
static void read_intra_frame_mode_info(AV1_COMMON *const cm,
  ...
  mbmi->mode =
#if CONFIG_EC_MULTISYMBOL
      read_intra_mode(r, get_y_mode_cdf(ec_ctx, mi, above_mi, left_mi, 0), AOM_DPRINTF_INFO_SBH, "intra_mode_y");
#else
  ...
  if (is_chroma_reference(mi_row, mi_col, bsize, xd->plane[1].subsampling_x,
    ...
    mbmi->uv_mode = read_intra_mode_uv(cm, xd, r, mbmi->mode, AOM_DPRINTF_INFO_SBH, "intra_mode_uv");
#else
...
static int read_mv_component(aom_reader *r, nmv_component *mvcomp, int usehp) {
  int mag, d, fr, hp;
  const int sign = aom_read(r, mvcomp->sign, ACCT_STR, AOM_DPRINTF_INFO_SBH, "mv_comp_sign");
  const int mv_class =
#if CONFIG_EC_MULTISYMBOL
      aom_read_symbol(r, mvcomp->class_cdf, MV_CLASSES, ACCT_STR, AOM_DPRINTF_INFO_SBH, "mv_comp_class");
#else
  ...
  // Integer part
  if (class0) {
    d = aom_read(r, mvcomp->class0[0], ACCT_STR, AOM_DPRINTF_INFO_SBH, "mv_comp_class0");
    mag = 0;
  } else {
    int i;
    ...
    d = 0;
    for (i = 0; i < n; ++i) d |= aom_read(r, mvcomp->bits[i], ACCT_STR, AOM_DPRINTF_INFO_SBH, "mv_comp_bits") << i;
...
// Fractional part
  ...
  fr = aom_read_symbol(r, class0 ? mvcomp->class0_fp_cdf[d] : mvcomp->fp_cdf,
                       MV_FP_SIZE, ACCT_STR, AOM_DPRINTF_INFO_SBH, class0 ? "mv_comp_class0_fp" : "mv_comp_fp");
#else
  ...
  hp = usehp ? aom_read(r, class0 ? mvcomp->class0_hp : mvcomp->hp, ACCT_STR, AOM_DPRINTF_INFO_SBH, class0 ? "mv_comp_class0_hp" : "mv_comp_hp")
             : 1;
...
static INLINE void read_mv(aom_reader *r, MV *mv, const MV *ref,
  ...
  joint_type =
#if CONFIG_EC_MULTISYMBOL
      (MV_JOINT_TYPE)aom_read_symbol(r, ctx->joint_cdf, MV_JOINTS, ACCT_STR, AOM_DPRINTF_INFO_SBH, "mv_joint");
#else
...
static REFERENCE_MODE read_block_reference_mode(AV1_COMMON *cm,
  ...
  if (cm->reference_mode == REFERENCE_MODE_SELECT) {
    ...
    const REFERENCE_MODE mode =
        (REFERENCE_MODE)aom_read(r, cm->fc->comp_inter_prob[ctx], ACCT_STR, AOM_DPRINTF_INFO_SBH, "comp_mode");
    FRAME_COUNTS *counts = xd->counts;
...
static void read_ref_frames(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (mode == COMPOUND_REFERENCE) {
      ...
      const int bit = aom_read(r, fc->comp_ref_prob[ctx][0], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_fwd_bit");
      ...
      if (!bit) {
        ...
        const int bit1 = aom_read(r, fc->comp_ref_prob[ctx1][1], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_fwd_bit1");
        ...
        const int bit2 = aom_read(r, fc->comp_ref_prob[ctx2][2], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_fwd_bit2");
      ...
      // Decode backward references.
      {
        ...
        const int bit_bwd =
            aom_read(r, fc->comp_bwdref_prob[ctx_bwd][0], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bwd_bit");
      ...
      const int bit0 = aom_read(r, fc->single_ref_prob[ctx0][0], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bit0");
      if (counts) ++counts->single_ref[ctx0][0][bit0];

      if (bit0) {
        const int ctx1 = av1_get_pred_context_single_ref_p2(xd);
        const int bit1 = aom_read(r, fc->single_ref_prob[ctx1][1], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bit1");
        if (counts) ++counts->single_ref[ctx1][1][bit1];
        ref_frame[0] = bit1 ? ALTREF_FRAME : BWDREF_FRAME;
      } else {
        const int ctx2 = av1_get_pred_context_single_ref_p3(xd);
        const int bit2 = aom_read(r, fc->single_ref_prob[ctx2][2], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bit2");
        if (counts) ++counts->single_ref[ctx2][2][bit2];
        if (bit2) {
          const int ctx4 = av1_get_pred_context_single_ref_p5(xd);
          const int bit4 = aom_read(r, fc->single_ref_prob[ctx4][4], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bit4");
          if (counts) ++counts->single_ref[ctx4][4][bit4];
          ref_frame[0] = bit4 ? GOLDEN_FRAME : LAST3_FRAME;
        } else {
          const int ctx3 = av1_get_pred_context_single_ref_p4(xd);
          const int bit3 = aom_read(r, fc->single_ref_prob[ctx3][3], ACCT_STR, AOM_DPRINTF_INFO_SBH, "ref_frame_bit3");
      ...
      if (has_subpel_mv_component(xd->mi[0], xd, dir) ||
        ...
        mbmi->interp_filter[dir] =
            (InterpFilter)av1_switchable_interp_inv[aom_read_symbol(
                ...
                ACCT_STR, AOM_DPRINTF_INFO_SBH, "interp_filter")];
#else
...
#if CONFIG_CB4X4
  (void)i;
  mbmi->mode = read_intra_mode_y(cm, xd, r, size_group_lookup[bsize], AOM_DPRINTF_INFO_SBH, "intra_mode_y");
#else
  switch (bsize) {
    case BLOCK_4X4:
      for (i = 0; i < 4; ++i)
...
static int read_is_inter_block(AV1_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    const int is_inter = aom_read(r, cm->fc->intra_inter_prob[ctx], ACCT_STR, AOM_DPRINTF_INFO_SBH, "intra_inter");
    FRAME_COUNTS *counts = xd->counts;
