...
static int parse_bitdepth_colorspace_sampling(BITSTREAM_PROFILE profile,
  ...
  color_space = (aom_color_space_t)aom_rb_read_literal(rb, 3, AOM_DPRINTF_INFO_NULL, NULL);
...
static aom_codec_err_t decoder_peek_si_internal(
...

  {
    ...
    const int frame_marker = aom_rb_read_literal(&rb, 2, AOM_DPRINTF_INFO_NULL, NULL);
    const BITSTREAM_PROFILE profile = av1_read_profile(&rb, AOM_DPRINTF_INFO_NULL);
    ...
    if (aom_rb_read_bit(&rb, AOM_DPRINTF_INFO_NULL, NULL)) {     // show an existing frame
      aom_rb_read_literal(&rb, 3, AOM_DPRINTF_INFO_NULL, NULL);  // Frame buffer to show.
      return AOM_CODEC_OK;
    }

    if (data_sz <= 8) return AOM_CODEC_UNSUP_BITSTREAM;

    si->is_kf = !aom_rb_read_bit(&rb, AOM_DPRINTF_INFO_NULL, NULL);
    show_frame = aom_rb_read_bit(&rb, AOM_DPRINTF_INFO_NULL, NULL);
    error_resilient = aom_rb_read_bit(&rb, AOM_DPRINTF_INFO_NULL, NULL);
#if CONFIG_REFERENCE_BUFFER
    {
      ...
      if (seq_params.frame_id_numbers_present_flag) {
        ...
        aom_rb_read_literal(&rb, frame_id_len, AOM_DPRINTF_INFO_NULL, NULL);
      }
    }
#endif
    if (si->is_kf) {
      if (!av1_read_sync_code(&rb, AOM_DPRINTF_INFO_NULL)) return AOM_CODEC_UNSUP_BITSTREAM;

      if (!parse_bitdepth_colorspace_sampling(profile, &rb))
        return AOM_CODEC_UNSUP_BITSTREAM;
      av1_read_frame_size(&rb, (int *)&si->w, (int *)&si->h, AOM_DPRINTF_INFO_NULL);
    } else {
      intra_only_flag = show_frame ? 0 : aom_rb_read_bit(&rb, AOM_DPRINTF_INFO_NULL, NULL);
