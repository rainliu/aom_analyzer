...
int16_t aom_read_primitive_symmetric_(aom_reader *r,
  ...
  if (aom_read_bit(r, ACCT_STR_NAME, level, desc)) {
    int s = aom_read_bit(r, ACCT_STR_NAME, level, desc);
    int16_t x = aom_read_literal(r, mag_bits, ACCT_STR_NAME, level, desc) + 1;
    return (s > 0 ? -x : x);
  } else {
    return 0;
  }
...
uint16_t aom_read_primitive_quniform_(aom_reader *r,
                                      uint16_t n ACCT_STR_PARAM, int level, const char* desc) {
  if (n <= 1) return 0;
...
uint16_t aom_read_primitive_refbilevel_(aom_reader *r, uint16_t n, uint16_t p,
  ...
  int v;
  if (aom_read_bit(r, ACCT_STR_NAME, level, desc)) {
    v = aom_read_primitive_quniform(r, p, ACCT_STR_NAME, level, desc) + lolimit;
  } else {
    v = aom_read_primitive_quniform(r, n - p, ACCT_STR_NAME, level, desc);
    if (v >= lolimit) v += p;
  }
  return v;
}
...
uint16_t aom_read_primitive_subexpfin_(aom_reader *r, uint16_t n,
  ...
  while (1) {
    ...
    int a = (1 << b);
    if (n <= mk + 3 * a) {
      v = aom_read_primitive_quniform(r, n - mk, ACCT_STR_NAME, level, desc) + mk;
      break;
    } else {
      if (aom_read_bit(r, ACCT_STR_NAME, level, desc)) {
        i = i + 1;
        ...
        v = aom_read_literal(r, b, ACCT_STR_NAME, level, desc) + mk;
        break;
      }
    }
  }
...
uint16_t aom_read_primitive_refsubexpfin_(aom_reader *r, uint16_t n, uint16_t k,
  ...
  return inv_recenter_finite_nonneg(
      n, ref, aom_read_primitive_subexpfin(r, n, k, ACCT_STR_NAME, level, desc));
}
...
int16_t aom_read_signed_primitive_refsubexpfin_(aom_reader *r, uint16_t n,
  ...
  return aom_read_primitive_refsubexpfin(r, scaled_n, k, ref, ACCT_STR_NAME, level, desc) -
         n + 1;
}
